  ...
  try {
    ...
    await elm.roles.add(berimute).then(() => {
      ...
      message.channel.send(
        `**${elm.user.tag}** telah selesai di ELM.\nAlasan : ${reason}`
      );
    });

    const embed = new EmbedBuilder()
      .setAuthor({
        name: `ELM | ${elm.user.tag}`,
        iconURL: elm.user.displayAvatarURL(),
      })
      .setColor("Random")
      .addFields(
    ...
    message.guild.channels.cache
      .find((c) => c.name === "ruang-bk")
      .send(
        `Hai **${elm}**, Selamat datang di <#932997960923480100>.
        member yang hanya bisa melihat channel ini artinya sedang dalam hukuman karena telah melanggar sesuatu. 
        Jika anda merasa pernah melakukan sesuatu yang melanggar rules, silahkan beritahu disini agar segera diproses oleh staff dan dapat melanjukan kembali aktivitas chat secara normal.`
      );
  } catch (error) {
