...
exports.run = async (client, message, args) => {
  try {
    ...
    if (durasi > 2419200000)
      return message.reply("durasi tidak boleh lebih dari 28 hari!");

    const reason = args.slice(2).join(" ") || "tidak diberi alasan";
    mutee.timeout(durasi, reason).then(() => {
      message.channel.send(
        `${mutee.user.tag} telah di mute selama ${timeout}!`
      );
    });
...
exports.help = {
  ...
  usage: "tempbisu <user> [reason]",
  example: "tempbisu @juned spam",
};
