...
exports.run = async (client, message, args) => {
    ...
    if (bannerUser) {
        ...
        if (mention) {
            ...
            embed
                .setAuthor({ name: userID.user.tag, iconURL: userID.user.displayAvatarURL({ size: 4096, dynamic: true }) })
                .setDescription(`[Banner URL](${bannerURL})`)
                .setImage(bannerURL);
        } else {
            embed
                .setAuthor({ name: message.author.tag, iconURL: message.author.displayAvatarURL({ size: 4096, dynamic: true }) })
                .setDescription(`[Banner URL](${bannerURL})`)
                .setImage(bannerURL);
        }
    } else if (mention) {
        embed.setAuthor({ name: mention.tag, iconURL: mention.displayAvatarURL({ size: 4096, dynamic: true }) })   //author embed
        ...
        embed.setAuthor({ name: userID.user.tag, iconURL: userID.user.displayAvatarURL({ size: 4096, dynamic: true }) })
        ...
        embed.setAuthor({ name: message.author.tag, iconURL: message.author.displayAvatarURL({ size: 4096, dynamic: true }) })
        ...
        const getUserBannerURL = message.guild.bannerURL({ size: 4096, dynamic: true });
        embed.setAuthor({ name: message.author.tag, iconURL: message.author.displayAvatarURL({ size: 4096, dynamic: true }) })
        embed.setDescription(`[Avatar URL Link](${getUserBannerURL})`)
        embed.setImage(getUserBannerURL)
    } else if (find) {
        embed.setAuthor({ name: find.user.tag, iconURL: find.user.displayAvatarURL({ size: 4096, dynamic: true }) })
