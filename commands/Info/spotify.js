...
exports.run = async (client, message, args) => {
    try {
        ...
        const member = await message.guild.members.cache.get(get.id).fetch();
        const presence = member.presence.activities;
        const getData = presence.find(a => a.name.toLowerCase() === 'spotify');
        ...
        const buttons = new ActionRowBuilder()
            .addComponents([
                new ButtonBuilder().setLabel('Get Preview Song ▶️').setStyle(ButtonStyle.Secondary).setCustomId(`spotify-preview-${get.id}`),
                new ButtonBuilder().setLabel('Download Song 📥').setStyle(ButtonStyle.Secondary).setCustomId(`spotify-download-${get.id}`)
            ]);
