...
exports.run = async (client, message) => {
  ...
  const offline = members.filter((m) => m.presence?.status == "offline").size,
    ...
    robot = members.filter((m) => m.user.bot).size,
    total = message.guild.memberCount;

  // Channels
  const channels = message.guild.channels;
  const text = channels.cache.filter(
      (r) => r.type === ChannelType.GuildText
    ).size,
    vc = channels.cache.filter((r) => r.type === ChannelType.GuildVoice).size,
    category = channels.cache.filter(
      (r) => r.type === ChannelType.GuildCategory
    ).size,
    forum = channels.cache.filter(
      (r) => r.type === ChannelType.GuildForum
    ).size,
    thread = channels.cache.filter(
      (r) => r.type === ChannelType.PublicThread
    ).size,
