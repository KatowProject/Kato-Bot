...
module.exports.run = async (bot, message, args) => {
    ...
    if (!rolenya) return message.reply("Tolong beritahu secara lengkap role yang ingin kauberikan!");
    ...
    if (membernya.roles.has(role.id)) return message.reply("Mereka sudah mempunyai role tersebut.");
    await membernya.addRole(role.id);
  
    try {
    await message.channel.send(`Selamat! Role ${role.name} sudah terpasang.`)
    }
    catch (e) {
        message.channel.send(`Selamat! Role ${role.name} sudah terpasang untuk <@${membernya.id}>. Tapi, saat kami mencoba mengirim pesan intim, mereka menguncinya.`)
    };
