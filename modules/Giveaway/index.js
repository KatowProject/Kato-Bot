...
const ms = require("ms");
const db = require("../../database/schemas/giveaway");
const dbxp = require("../../database/schemas/xp");
...
module.exports = class Giveaway {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let msg = await message.channel.send(
          "Pilih channel yang ingin dijadikan tempatnya!"
        );
          ...
          switch (status) {
            case "channel":
              ...
              msg = await message.channel.send(
                ...
                }> menjadi tempat Giveaway, selanjutnya silahkan masukkan durasi Giveaway!`
              );
              break;
            ...
            case "time": {
              ...
              status = "winner";
              msg = await message.channel.send(
                `Giveaway berdurasi **${this.client.util.parseDur(
                  data.time
                )}**, selanjutnya masukkan jumlah pemenang!`
              );
              break;
            }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        collector.on("collect", async (m) => {
          ...
          let msg =
            data.values === "MEE6"
              ? await message.reply("Masukkan level yang dibutuhkan!")
              : data.values === "ROLE"
              ? await message.reply(
                  "Silahkan masukkan role yang ingin dimasukkan (Name, RoleID)\n**Contoh**: santai dermawan, nitro boost, santai"
                )
          ...
          const collector2 = message.channel.createMessageCollector({
            filter: (c) => c.author.id === message.author.id,
            time: 69_000,
          });
          collector2.on("collect", async (m) => {
            switch (data.values) {
              case "MEE6":
                if (!parseInt(m.content))
                  return message.reply("Harus bernilai angka!");
              ...
              case "ROLE": {
                ...
                for (const role of roles) {
                  ...
                  const findRole = message.guild.roles.cache.find(
                    (a) => a.name.toLowerCase() === name.toLowerCase()
                  );
                  const roleRegex = new RegExp(name, "i");
                  const findRegex = message.guild.roles.cache.find((a) =>
                    roleRegex.test(a.name) ? roleRegex.test(a.name) : null
                  );
              ...
              case "title":
                if (m.content.length < 10)
                  return message
                    .reply("Setidaknya isi Judul sebanyak 10 karakter!")
                    .then((t) => setTimeout(() => t.delete(), 5000));
                msg.delete();
                collector2.stop();
  ...
  _createFinal(message, data) {
    return new Promise(async (resolve) => {
      const embed = new EmbedBuilder()
        ...
        .addFields({ name: "Dibuat oleh:", value: `<@${message.author.id}>` })
        .setFooter({ text: `${data.winner} pemenang | Berakhir pada` })
        .setTimestamp(Date.now() + data.time);

      data.require.name === "MEE6"
        ? embed.addFields({
            name: "Dibutuhkan:",
            value: `Level MEE6 - **${data.require.value}**`,
          })
        : data.require.name === "ROLE"
        ? embed.addFields({
            name: "Dibutuhkan:",
            value: `Role ${data.require.value
              .map((a) => `<@&${a}>`)
              .join(", ")}`,
          })
      ...
      const r = await message.channel.send({
        content: "Apakah kamu sudah yakin?",
        embeds: [embed],
        components: [buttons],
      });
      const collector = r.channel.createMessageComponentCollector({
      ...
      collector.on("collect", async (m) => {
        ...
        switch (m.customId) {
          case `y-${message.id}`: {
            ...
            const giveaway = await message.guild.channels.cache
              .get(data.channel)
              .send({
                content: "🎉- Giveaway -🎉",
                embeds: [embed],
  ...
  getData(message, args) {
    return new Promise(async (resolve, reject) => {
      try {
        const giveawayDataList = await db.find({}).sort({ time: -1 });
        if (!giveawayDataList.length) return message.reply("Nothing data!");

        const mapData = giveawayDataList.map(
          (a) =>
            `[${a.messageID}](https://discord.com/channels/${a.guildID}/${
              a.channelID
            }/${a.messageID}) - ${a.embed.description} | ${
              a.isDone ? "Telah Selesai" : "Belum Selesai"
            }`
        );
        ...
        collector.on("collect", async (i) => {
          switch (i.customId) {
            case args[0] + "back" + message.id:
              ...
              embede.setDescription(chunkData[pagination - 1].join("\n"));
              embede.setFooter({
                text: `Page ${pagination} of ${chunkData.length}`,
              });
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (giveawayDataEnd.isDone)
          return message.reply("Giveaway already ended!");

        giveawayDataEnd.time.duration = 0;
        await db.findOneAndUpdate(
          { messageID: giveawayDataEnd.messageID },
          giveawayDataEnd
        );

        message
          .reply("Telah selesai diberhentikan, tunggu 1-10 detik ya!")
          .then((t) => setTimeout(() => t.delete(), 5000));
  ...
  delete(message, args) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!giveawayDataDelete.isDone)
          return message
            .reply(`Giveaway isn't over yet, pls end giveaway first!`)
      ...
      if (timeLeft > data.time.duration) {
        ...
        if (data.entries.length === 0) {
          ...
          return this.client.channels.cache
            .get(data.channelID)
            .send(`Tidak ada yang menang karna nol partisipan!`);
        }
        ...
        this.client.channels.cache.get(data.channelID).send({
          content: `Congrats ${winners.map((a) => `<@${a}>`).join(", ")}!\n${
            msg.url
          }`,
  ...
  giveawayValidator(interaction) {
    return new Promise(async (resolve) => {
      ...
      if (data.entries.includes(interaction.user.id))
        return interaction.user.send("Sudah terdaftar dalam giveaway!");

      let isOK = null;
      switch (data.require.name) {
        case "MEE6": {
          const xps = await dbxp.findOne({ id: interaction.guild.id });
          if (!xps) return;
          const xp = xps.data;
          const player = xp.find((a) => a.id === interaction.user.id);
          if (!player)
            return interaction.user.send(
              "Tidak dapat mengikuti karna Level tidak mencukupi!"
            );
      ...
      interaction.followUp({
        content: "Berhasil terdaftar dalam giveaway!",
        ephemeral: true,
      });
      resolve();
    });
   ...
   * @param {ButtonInteraction} interaction
   */
  async giveawayCancel(interaction) {
    ...
    if (data.isDone) return interaction.user.send("Giveaway telah berakhir!");
    if (!data.entries.includes(interaction.user.id))
      return interaction.user.send("Tidak terdaftar dalam giveaway!");
    ...
    interaction.followUp({
      content: "Berhasil keluar dari giveaway!",
      ephemeral: true,
    });
  }
